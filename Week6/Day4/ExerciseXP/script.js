...
const colors = ["Blue", "Green", "Red", "Orange", "colors", "Indigo", "Yellow"];
...
function showColors1(){
    colors1.forEach((color, index) => {
            const ordinalSuffix = ordinal[index < 3 ? index + 1 : 0]; 
...
// my prediction []
// wrong, output is [ undefined, undefined ]
...
const namesFullStack = users.filter((element) => {
    if (element.role == 'Full Stack Resident') {
       return element
    }; 
}).map((element) => {
...
const text = epic.reduce ((acc, elem) => {
    return `${acc + elem} `;
},)
...
const studentPassedCongrats = students.filter((student) => {
    if (student.isPassed == true) {
        return student
    }
}).forEach((student) => {
    console.log (`Good job ${student.name}, you passed the course in I${student.course}`)
})
